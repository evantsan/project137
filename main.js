        ...
        if(stat != "")
    {
        ...
        for (var i=0;i < objects.length;i++) 
        {
            ...
            if(objects[i].label == inpu)
            {
                ...
                speak_data3 = "I repeat "+inpu+"found"
                var ut = new SpeechSynthesisUtterance(speak_data1+speak_data1+speak_data3);
                synth.speak(ut); 
                
            }
            else if(abc == 'round 1'){
                document.getElementById("statl").innerHTML = "Status : "+inpu+" not found";
                var synth = window.speechSynthesis;
                speak_data2 = inpu+" not found";
            ...
            else if(abc == 'round 2')
            {
                document.getElementById("statl").innerHTML = "Status : "+inpu+" not found";
                var synth = window.speechSynthesis;
                speak_data2 = "I repeat "+inpu+" not found";
                var ut = new SpeechSynthesisUtterance(speak_data2);
                synth.speak(ut); 
                abc = 'round 3'
                console.log("IMPORTANT: "+abc);
            ...
            else if(abc == 'round 3')
            {
                document.getElementById("statl").innerHTML = "Status : "+inpu+" not found";
                var synth = window.speechSynthesis;
                speak_data2 = "For the final time "+inpu+" not found";
                var ut = new SpeechSynthesisUtterance(speak_data2);
                synth.speak(ut); 
                abc = 'round 4'
                console.log("IMPORTANT: "+abc);
